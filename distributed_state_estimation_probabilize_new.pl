...
% prob(msg(gc, co, ping), Set, 0.9) :-
%   subset(Set, [msg(gc, co, ping), msg(gc, co, low), msg(gc, co, max), msg(gc, co, stop)]),
%   subset([msg(gc, co, ping), msg(gc, co, low), msg(gc, co, max), msg(gc, co, stop)], Set).
% prob(msg(gc, co, low), Set, 0.02) :-
%   subset(Set, [msg(gc, co, ping), msg(gc, co, low), msg(gc, co, max), msg(gc, co, stop)]),
%   subset([msg(gc, co, ping), msg(gc, co, low), msg(gc, co, max), msg(gc, co, stop)], Set).
% prob(msg(gc, co, max), Set, 0.05) :-
%   subset(Set, [msg(gc, co, ping), msg(gc, co, low), msg(gc, co, max), msg(gc, co, stop)]),
%   subset([msg(gc, co, ping), msg(gc, co, low), msg(gc, co, max), msg(gc, co, stop)], Set).
% prob(msg(gc, co, stop), Set, 0.03) :-
%   subset(Set, [msg(gc, co, ping), msg(gc, co, low), msg(gc, co, max), msg(gc, co, stop)]),
%   subset([msg(gc, co, ping), msg(gc, co, low), msg(gc, co, max), msg(gc, co, stop)], Set).
% prob(msg(gc, co, keep), Set, 0.15) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, max), msg(gc, co, stop)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, max), msg(gc, co, stop)], Set).
% prob(msg(gc, co, revert), Set, 0.2) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, max), msg(gc, co, stop)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, max), msg(gc, co, stop)], Set).
% prob(msg(gc, co, max), Set, 0.35) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, max), msg(gc, co, stop)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, max), msg(gc, co, stop)], Set).
% prob(msg(gc, co, stop), Set, 0.3) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, max), msg(gc, co, stop)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, max), msg(gc, co, stop)], Set).
% prob(msg(gc, co, keep), [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, stop)], 0.5) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, stop)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, stop)], Set).
% prob(msg(gc, co, revert), Set, 0.06) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, stop)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, stop)], Set).
% prob(msg(gc, co, low), Set, 0.3) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, stop)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, stop)], Set).
% prob(msg(gc, co, stop), Set, 0.14) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, stop)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, stop)], Set).
% prob(msg(gc, co, keep), Set, 0.35) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, max)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, max)], Set).
% prob(msg(gc, co, revert), Set, 0.45) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, max)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, max)], Set).
% prob(msg(gc, co, low), Set, 0.12) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, max)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, max)], Set).
% prob(msg(gc, co, max), Set, 0.08) :-
%   subset(Set, [msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, max)]),
%   subset([msg(gc, co, keep), msg(gc, co, revert), msg(gc, co, low), msg(gc, co, max)], Set).
%
%
% % Example from Paper
% trace_expression(cogc, COGC) :-
%   COGC = AA,
%   AA = ((msg(gc, co, ping) : AA) \/ (msg(gc, co, stop) : STA) \/ (msg(gc, co, low) : LS) \/ (msg(gc, co, max) : MS)),
%   STA = ((msg(gc, co, keep) : STA) \/ (msg(gc, co, revert) : AA) \/ (msg(gc, co, low) : LS) \/ (msg(gc, co, max) : MS)),
%   LS = ((msg(gc, co, keep) : LS) \/ (msg(gc, co, revert) : AA) \/ (msg(gc, co, stop) : STA) \/ (msg(gc, co, max) : MS)),
%   MS = ((msg(gc, co, keep) : MS) \/ (msg(gc, co, revert) : AA) \/ (msg(gc, co, stop) : STA) \/ (msg(gc, co, low) : LS)).


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% prob predicate is used to associate probabilities to messages
% general use: prob(<msg>, <set of msg options>, <probability>)
% example: prob (msg1, [msg1, msg2, msg3], 0.6) means that, when in the protocol
% we can consume msg1, if in the current state the possible messages are msg1, msg2 and
% msg3, then the probability to observe msg1 is 60%.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% if we do not specify any specific probability for the messages, by default all
% the messages are always equiprobable
prob(Ev, Set, Prob) :-
  member(Ev, Set),
  length(Set, N),
  Prob is 1 / N, !.
...
% predicate to extract the set of msg options in the current state T
% example: extract_events((msg1:epsilon) \/ (msg2:epsilon), [msg1, msg2])
extract_events(T, Set) :-
  empty_assoc(A),
  extract_events(T, Set, A).
extract_events(T, [], A) :-
  get_assoc(T, A, _), !.
extract_events(epsilon, [], _).
extract_events(ET:T, Set, A) :-
  put_assoc(ET:T, A, _, A1),
  extract_events(T, Set1, A1),
  (member(ET, Set1) -> Set = Set1; append([ET], Set1, Set)).
extract_events(ET>>T, Set, A) :-
  put_assoc(ET>>T, A, _, A1),
  extract_events(T, Set, A1).
extract_events(T1\/T2, Set, A) :-
  put_assoc(T1\/T2, A, _, A1),
  extract_events(T1, Set1, A1),
  extract_events(T2, Set2, A1),
  union(Set1, Set2, Set).
extract_events(T1|T2, Set, A) :-
  put_assoc(T1|T2, A, _, A1),
  extract_events(T1, Set1, A1),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% predicate to get the probability of a msg; since the probability is not defined
% inside the trace expression anymore, we need this auxiliary predicate to retrieve
% it at runtime when we need it. The probability is obtained through the predicate
% prob, and it requires to first extract all the messages which are possible in the
% current state T
get_prob(T, ET, Prob) :-
  findall(
    Ev, (
...
next_p(ET:T, gap(Ev), (T, Prob), CurrentState) :-
  match(Ev, ET),
  get_prob(CurrentState, ET, Prob).
next_p(ET:T, gap(Ev), (T, 1), CurrentState) :-
  match(Ev, ET),
  not(get_prob(CurrentState, ET, _)).
next_p(ET:T, Ev, (T, 1), _) :- % it depends on how it will be generated from the HMM
  var(Ev) -> match(Ev, ET); Ev \= gap(_), match(Ev, ET).
next_p(T1\/_, Ev, (T2, Prob), CurrentState) :-
  next_p(T1, Ev, (T2, Prob), CurrentState).
next_p(_\/T1, Ev, (T2, Prob), CurrentState) :-
  !, next_p(T1, Ev, (T2, Prob), CurrentState).
next_p(T1|T2, Ev, (T, Prob), CurrentState) :-
  next_p(T1, Ev, (T3, Prob), CurrentState),
  fork(T3, T2, T).
next_p(T1|T2, Ev, (T, Prob), CurrentState) :-
  !, next_p(T2, Ev, (T3, Prob), CurrentState),
  fork(T1, T3, T).
next_p(T1*T2, Ev, (T, Prob), CurrentState) :-
  next_p(T1, Ev, (T3, Prob), CurrentState),
  concat(T3, T2, T).
next_p(T1*T2, Ev, (T3, Prob), CurrentState) :-
  !, may_halt(T1),
  next_p(T2, Ev, (T3, Prob), CurrentState).
next_p(T1/\T2, Ev, (T, Prob), CurrentState) :-
  !, next_p(T1, Ev, (T3, Prob1), CurrentState),
  next_p(T2, Ev, (T4, Prob2), CurrentState),
...
next_p((T1, Prob1), Ev, Ev1, (T2, Prob2)) :-
  ...
  (
    ...
    (next_p(T1, Ev, (T2, Prob3), T1), Prob2 is Prob1 * Prob3, Ev1 = Ev)
  ).
